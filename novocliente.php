...
$proc = mysql_query("SELECT * FROM clientes WHERE nome ='$nome'");



   $select = $pdo->query("SELECT * FROM clientes WHERE nome ='$nome'")->fetchAll();
   ...
   if ( $nome == "" || $convenio == "" ) {
	 ...
	 echo"<script language='javascript' type='text/javascript'>alert('O campo Nome e Convenio deve ser preenchido');</script>";
...
if($statement->execute()){
  ...
  echo '<script> window.close(); </script>';
	  ...
	  echo '<script>alert("0.0! Erro ao Cadastrar - usuario já Cadastrado "); </script>'; 
  echo '<script> window.close(); </script>';
...
// Seleciona o Banco de dados através da conexão acima
                 ...
                 $resultado = $conn->query($sql);
				   ...
				   echo '<option name =nome value='.$row["id_convenios"].'>'.utf8_encode($row["nome_convenio"]).'</option>';
